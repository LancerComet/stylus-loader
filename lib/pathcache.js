...
function resolvers(options, webpackResolver) {
  ...
  var webpackResolve = function (context, pathName) {
    ...
    if (true) {
      ...
      let resultPath = context

      // 查询当前文件夹是否有 node_modules.
      while (!stopLooking) {
        const findNodeModule = isPathExist(path.join(resultPath, 'node_modules'))
        if (!findNodeModule) {
      ...
      replacePath = path.join(resultPath, 'node_modules', pathName)
      usingCustomPath && console.log(replacePath)
    }

    pathName = loaderUtils.urlToRequest(pathName, options.root);

    // First try with a '.styl' extension.
    return whenWebpackResolver(context, pathName + '.styl')
      ...
      .catch(function () { return whenWebpackResolver(context, pathName); })
      .catch(function () { return null; })
      .then(function (result) {
        const finalPath = Array.isArray(result) && result[1] && result[1].path || result
        if (replacePath && usingCustomPath) {
          return replacePath
        } else {
          return finalPath
        }
