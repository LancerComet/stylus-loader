...
function resolvers(options, webpackResolver) {
  ...
  var webpackResolve = function (context, pathName) {
    ...
    let replacePath = ''
    let usingCustomPath = false

    // 不带 ~ 开头的 path 查询目录.
    // TODO: Configuable.
    if (true) {
      const filteredPath = pathName.replace('~', '')  // 去掉 "~" 拼接.

      // 目标 node_modules 路径.
      let stopLooking = false
      let resultPath = path.normalize(context)

      // 查询当前文件夹是否有 node_modules.
      while (!stopLooking) {
        const findNodeModule = isPathExist(path.join(resultPath, 'node_modules'))  // 查找当前目录有没有 node_modules.
        if (!findNodeModule) {
          const parentPath = getParentPath(resultPath)
          if (parentPath === resultPath) {
            stopLooking = true
          }
      ...
      replacePath = path.join(resultPath, 'node_modules', filteredPath)
    }

    const originalPath = loaderUtils.urlToRequest(pathName, options.root);

    // First try with a '.styl' extension.
    return whenWebpackResolver(context, originalPath + '.styl')
      ...
      .then(function (result) {
        const finalPath = Array.isArray(result) && result[1] && result[1].path || result
        if (usingCustomPath && replacePath && isPathExist(replacePath)) {
